...
export const createLoginLink = actionClient
  ...
  .action(async ({ parsedInput }) => {
    ...
    if (!recipient) throw new Error("Conta não configurada ou não ativa");

    // Gera um link de login único para o dashboard Express do Stripe.
    // O organizador gerencia saques, extrato e conta bancária diretamente pelo Stripe.
