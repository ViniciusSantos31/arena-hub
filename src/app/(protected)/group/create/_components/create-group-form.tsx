...
import { useCreateGroupPlan } from "./create-group-plan-context";
import { GroupFeedCardPreview } from "./group-card-preview";

export function CreateGroupForm() {
  const { maxPlayersLimit } = useCreateGroupPlan();
  const maxPlayersCap = maxPlayersLimit ?? 100;
  ...
  const methods = useForm({
    ...
    defaultValues: {
      name: "",
      description: "",
      maxPlayers: Math.min(10, maxPlayersCap),
      rules: "",
    },
  });
  ...
  const createGroupAction = useAction(upsertGroup, {
    ...
    onError({ error }) {
      toast.error(error.serverError ?? "Não foi possível criar o grupo.");
    },
  });
              ...
              <div className="flex-1 space-y-2">
                    ...
                    <div className="flex w-full flex-1">
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          if (value > maxPlayersCap) {
                            e.target.value = String(maxPlayersCap);
                            return;
                          }
                ...
                {maxPlayersLimit != null && (
                  <p className="text-muted-foreground text-xs">
                    Seu plano permite até {maxPlayersLimit} membros por grupo.
                  </p>
                )}
              </div>
